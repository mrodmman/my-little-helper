/**
 * /intel layout — hub at /intel, articles at /intel/$slug.
 * Uses useLocation().pathname to switch between hub and article rendering;
 * pathname is identical on server and client so hydration stays deterministic.
 */
...
function IntelLayout() {
  ...
  // pathname is set from the request URL — same value on server and client.
  const isHub = pathname === "/intel" || pathname === "/intel/";
  return isHub ? <IntelHub /> : <Outlet />;
}
    ...
    <div className="min-h-screen" style={{ background: "#F4F6FA" }}>
      ...
      <div id="articles" className="max-w-6xl mx-auto px-4 sm:px-6 py-12">
        ...
        <div className="mt-16 rounded-2xl bg-[#0D1220] p-8 text-center">
          <div className="text-white text-2xl font-bold mb-2">Get the free build kits.</div>
