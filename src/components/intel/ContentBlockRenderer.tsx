...
function BlockSwitch({ block }: { block: ContentBlock }) {
  switch (block.type) {
    ...
    case "heading":
      return (
        <h2 className="text-2xl font-bold text-[#0D1220] mt-8 mb-3 leading-tight">{block.text}</h2>
      );
          ...
          {block.items.map((item, i) => {
            const text =
              typeof item === "string"
                ...
                : typeof item === "object" && item !== null
                  ? [
                      "title" in item ? String((item as { title?: unknown }).title ?? "") : "",
                      "text" in item ? String((item as { text?: unknown }).text ?? "") : "",
                    ]
                      .filter(Boolean)
                      .join(": ")
                  : "";
            return (
              <li key={i} className="flex items-start gap-3">
                ...
                <span className="text-[#2D3748] leading-relaxed pt-0.5">{text}</span>
              </li>
            );
          })}
        </ol>
      ...
      return (
        <div className="rounded-xl bg-[#F4F6FA] border border-[#C8C3BA]/60 p-4 flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="font-semibold text-[#0D1220] text-sm">
              {block.toolName ?? (block as { tool_name?: string }).tool_name ?? "Tool"}
            </div>
            <p className="text-[#556070] text-sm mt-0.5">{block.description ?? ""}</p>
            {(block.url ?? (block as { cta_url?: string }).cta_url) && (
              <a
                href={block.url ?? (block as { cta_url?: string }).cta_url}
                target="_blank"
      ...
      return (
        <div className="rounded-xl bg-[#2563FF]/5 border-2 border-[#2563FF]/20 p-5 text-center">
          <div className="font-bold text-[#0D1220] text-lg mb-1">
            {block.headline ?? (block as { heading?: string }).heading ?? "Learn more"}
          </div>
          <p className="text-[#556070] text-sm mb-4">
            {block.description ?? (block as { text?: string }).text ?? ""}
          </p>
          <a
            href={block.buttonUrl ?? (block as { button_url?: string }).button_url ?? "#"}
          ...
          >
            {block.buttonText ??
              (block as { button_text?: string; cta_text?: string }).button_text ??
              (block as { cta_text?: string }).cta_text ??
              "Continue"}
          </a>
        </div>
      );
    ...
    case "quote":
      return (
        <blockquote className="border-l-4 border-[#2563FF] pl-4 py-1">
          ...
          {block.author && (
            <cite className="text-[#556070] text-sm not-italic mt-1 block">— {block.author}</cite>
          )}
        </blockquote>
      );
...
function CalloutBlock({ block }: { block: Extract<ContentBlock, { type: "callout" }> }) {
  const variant =
    block.variant ?? (block as { style?: "info" | "warning" | "tip" | "success" }).style ?? "info";
  const configs = {
    info: {
      icon: Info,
      bg: "bg-blue-50",
