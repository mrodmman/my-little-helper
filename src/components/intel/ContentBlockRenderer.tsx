...
function BlockSwitch({ block }: { block: ContentBlock }) {
  ...
  switch (block.type) {
            ...
            return (
              <li key={i} className="flex items-start gap-3">
                ...
                <div className="flex-1 pt-0.5">
                  {title && (
                    <div className="font-semibold text-[#0D1220] text-sm mb-1">{title}</div>
                  )}
    ...
    case "comparison_cards": {
      const rawBlock = block as typeof block & {
        label?: string;
        cards?: Array<{ title?: string; items?: string[]; variant?: "pro" | "con" | "neutral" }>;
      };
      const cards = asArray(rawBlock.cards);
      if (cards.length === 0) return null;
      return (
        <div className="space-y-3">
          {rawBlock.label && (
            <p className="text-xs font-bold uppercase tracking-wider text-[#556070]">
              {rawBlock.label}
            </p>
          )}
